<!--
  ifco —— 区级实施进度管理（/ifco/impl-progress/district）

  实施进度跟踪 · 区级端。顶部四张统计卡（倒排工期计划确认/月度进度确认/
  片区三色图进展/提示督办，卡即单选 select——点选切换下方列表，经路由
  ?card= 持久化，无参数默认倒排工期计划确认）+ 四个列表面板（v-show 切换
  不销毁）。卡片形式对齐在库项目管理 list。

  面板操作：倒排/月度确认 = 查看（表单抽屉只读）+ 行内 确认/退回（退回写
  退回意见，填报端呈退回修改态）；片区三色图进展 = 行内 三色图进展 评估
  （红/黄/绿）；提示/督办 = 整单处理抽屉（下发信息只读 + 涉及片区和项目的
  处理情况 + 区级处理情况）。

  菜单注册（菜单名称「区级实施进度管理」）：
   - 链接地址：/ifco/impl-progress/district
   - 组件位置：/ifco/impl-progress/district/index（与链接地址一致）
-->
<template>
  <PageWrapper contentClass="flex flex-col gap-16px">
    ...
    <ConfirmPanel v-show="selectedCard === 'confirm'" />
    <MonthlyConfirmPanel v-show="selectedCard === 'monthly-confirm'" />
    <TriColorPanel v-show="selectedCard === 'tricolor'" />
    <SupervisePanel v-show="selectedCard === 'supervise'" />
  </PageWrapper>
</template>
<script lang="ts" setup name="ViewsIfcoImplProgressDistrict">
  import { computed } from 'vue';
  import { useRoute, useRouter } from 'vue-router';
  import { Progress } from 'antdv-next';
  import { PageWrapper } from '@jeesite/core/components/Page';
  import { DISTRICT_CARDS, type DistrictCardKey } from '@jeesite/ifco/api/ifco/impl-progress';
  import ConfirmPanel from './confirm-panel.vue';
  ...
  /** 默认选中倒排工期计划确认（URL 无 ?card= 参数时同样按其过滤） */
  const DEFAULT_CARD: DistrictCardKey = 'confirm';

  const selectedCard = computed<DistrictCardKey>(() => {
    const card = route.query.card;
    return DISTRICT_CARDS.some((item) => item.key === card) ? (card as DistrictCardKey) : DEFAULT_CARD;
  });
