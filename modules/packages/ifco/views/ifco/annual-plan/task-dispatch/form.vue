<!--
  ...
  表单分两区（FormGroup）：市级任务要求（任务年份/创建时间/编制开始时间/
  市级编制结束时间/年度刚性目标说明/年度刚性投资目标）+ 区级任务分解要求
  （区级编制结束时间/各区年度刚性投资目标，均一排一个整行）。
  底部按钮 取消/提交（确定按钮 okText=提交）；提交校验必填项。
  当前后端尚未介入：保存仅组装数据后关闭抽屉、emit success 交 list.vue 写回内存。
-->
<template>
  <BasicDrawer
    v-bind="$attrs"
...
<script lang="ts" setup name="ViewsIfcoAnnualPlanTaskDispatchForm">
  ...
  import { DISTRICTS, newTaskId, taskYearOptions, type TaskDispatchItem } from '@jeesite/ifco/api/ifco/task-dispatch';
  ...
  const inputFormSchemas: FormSchema[] = [
    ...
    },
    {
      ...
      componentProps: { valueFormat: 'YYYY-MM-DD', style: 'width: 100%', placeholder: '请选择创建时间' },
    },
    {
      label: '编制开始时间',
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data: any) => {
    ...
    await setFieldsValue({
      ...
      createTime: record.value.createTime ?? '',
  ...
  async function handleSubmit() {
    ...
    const payload: TaskDispatchItem = {
      id: record.value.id ?? newTaskId(),
