...
6. **projects 回显含存量导入行（后端 2026-09-17 起）**：form 接口返回 存量行（`fillFlag:"0"`，排前，
   带 `id`/`pUid` 透传键）+ 填报行（`fillFlag:"1"`）全量。2026-09-18 起存量行**可编辑可删除**
   （tab 仅保留「存量」来源徽标），**保存值包含存量行并带回 pUid/fillFlag**（被删的不在数组中）；
   后端配套前其保存仍跳过 fillFlag=0 行（编辑/删除暂不落库，无重复风险），配套需求见
   变更说明 §10；存量行日期已截断 yyyy-MM。tab 标题显示项目名称（未填回退「项目N」）。
