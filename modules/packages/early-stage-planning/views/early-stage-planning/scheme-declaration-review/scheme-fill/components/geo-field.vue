<!--
  ...
  —— 本组件负责双向换算，对区块暴露 v-model:value（WKT 字符串）：
   ...
   - 上传：POST /a/esp/schemeFill/parseVector（dwg/shp(zip 或配套)/geojson），
     返回 wkt → GeoJSON 展示；
...
<template>
  <GeoDataSection
    ...
    :accept="ACCEPT"
    upload-text="上传 shp / dwg / geojson 文件"
...
<script lang="ts" setup name="ViewsEarlyStagePlanningSchemeFillGeoField">
  ...
  /** 后端支持的解析形态：geojson 纯 Java 解析；dwg / shp（配套同批或 zip 打包）依赖服务器 GDAL */
  const ACCEPT = '.shp,.dwg,.geojson,.json,.zip';
  ...
  /** 上传解析：后端返回 wkt → 还原 GeoJSON 供地图展示与编辑（存储值经上方 watch 统一回写）；还原失败抛错提示且不动已有数据 */
  async function parseFile(file: File): Promise<string> {
    const parsed = await schemeFillParseVector(file);
