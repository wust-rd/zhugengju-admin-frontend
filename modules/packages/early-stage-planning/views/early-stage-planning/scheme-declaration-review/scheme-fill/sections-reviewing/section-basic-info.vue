<!--
  填报页区块一 · 待审查片区版（Tab② 待审查片区填报，isApprove=2 专用）

  与已批准片区版的唯一字段差异：本版为「申报年份」（年份选择 YYYY，选填），
  没有片区批次；已批准版为「片区批次」（见 ../sections-approved/section-basic-info.vue）；
  其余字段完全一致，为方便对照阅读刻意两份独立维护，不走复用分支。
  申报年份与批次同存 batch 键（后端无需感知差异）。

  布局：垂直标签（label 在上）+ 两列栅格，概况/图片/范围/范围线通栏。
  字段：片区名称* / 申报年份 / 行政区* / 片区规模（公顷）* / 起止时间（年月区间）/
...
<script lang="ts" setup name="ViewsEarlyStagePlanningSchemeFillReviewingSectionBasicInfo">
