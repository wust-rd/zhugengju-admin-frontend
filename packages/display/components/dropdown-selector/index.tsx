 ...
 *
 * 注：本文件自 dev 分支回移；dev 版默认清除图标用 lucide-vue-next 的 CircleX，
 * 本分支未引入该依赖，改用等价的 iconify 图标（i-ri-close-circle-fill，与下方用法示例一致）。
 *
 * 提取自「城市体检」页下拉的共性：
        ...
        >
          ...
          {props.allowClear && hasValue.value && (
            <div class="ml-auto flex items-center cursor-pointer" onClick={handleClear}>
              {slots.clearIcon?.() ?? <div class="i-ri-close-circle-fill size-20px text-gray-400" />}
            </div>
          )}
