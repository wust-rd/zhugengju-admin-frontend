...
/** 抽屉 Tab 配置 */
const DRAWER_TABS = ['基本情况', '体检情况', '功能策划', '项目情况', '实施后评估'] as const;
  ...
  setup() {
    ...
    return () => (
        ...
        >
          ...
          <div class="flex h-44px items-stretch bg-[#1a3a5c]">
            {DRAWER_TABS.map((tab) => (
              <div
                key={tab}
                class={
                  'flex flex-1 cursor-pointer items-center justify-center text-14px whitespace-nowrap transition-all duration-100 ' +
                  (activeTab.value === tab
          ...
          {/* 内容区：5 个 Tab 的内容按顺序排列，点击 Tab 滚动定位到对应区块 */}
