...
/**
 * SearchFilter —— 搜索筛选行：搜索框 + 全部筛选下拉
 *
 * 搜索输入框（lucide 搜索前缀，清除按钮换为 lucide X）；
 * 右侧"全部"筛选下拉（允许清空）。
 *
 * props：
 * - value: 搜索框关键字（v-model:value 双向绑定）
 * - placeholder: 搜索框占位文本
 * - items: 筛选下拉选项
    ...
    return () => (
      <div class="flex items-center gap-x-12px h-40px">
        ...
        <Input
          value={props.value}
          classes={{
            root: '!bg-white/6 !border-gray-500 focus-within:!border-cyan-500 flex-1 h-40px text-white !rd-8px',
            input: 'placeholder:!text-gray-500 !pl-4px',
          }}
          prefix={<Search class="size-20px text-gray-400" />}
          placeholder={props.placeholder}
          allowClear
          onUpdate:value={(v: any) => emit('update:value', String(v ?? ''))}
        >
          {{
        ...
        <DropdownSelector
          ...
          items={props.items}
          placeholder="全部"
          class="w-136px rd-8px px-12px shrink-0"
          allowClear
