...
declare class SourceCache extends Evented {
  ...
  _updateRetainedTiles(
    idealTileIDs: Array<OverscaledTileID>,
    zoom: number,
  ): {
    [_: string]: OverscaledTileID;
  };
  ...
  ): import('@maplibre/maplibre-gl-style-spec').FeatureState;
  /**
...
export type SymbolLayoutProps = {
  ...
  'text-variable-anchor': DataConstantProperty<
    Array<'center' | 'left' | 'right' | 'top' | 'bottom' | 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right'>
  >;
...
declare function loadGlyphRange(
  fontstack: string,
  range: number,
  urlTemplate: string,
...
declare class GlyphManager {
  ...
  getGlyphs(glyphs: { [stack: string]: Array<number> }): Promise<GetGlyphsResponse>;
...
declare class Terrain {
  ...
  _getOverscaledTileIDFromLngLatZoom(
    lnglat: LngLat,
    zoom: number,
  ): {
    tileID: OverscaledTileID;
...
declare class ImagePosition {
  ...
  constructor(
    paddedRect: Rect,
    { pixelRatio, version, stretchX, stretchY, content, textFitWidth, textFitHeight }: StyleImage,
  );
...
declare class CollisionIndex {
  ...
  projectPathToScreenSpace(
    projectedPath: Array<Point>,
...
declare class Placement {
  ...
  storeCollisionData(
    ...
    placedGlyphBoxes: PlacedBox,
    placedIconBoxes: PlacedBox,
    placedGlyphCircles: PlacedCircles,
  ): void;
...
/**
 * 超图增强（maplibre-gl-enhance.js）：坐标系（CRS）类。
 * 支持 CGCS2000（EPSG:4490）、北京54（EPSG:4214）、西安80（EPSG:4610）等自定义坐标系，
 * 通过内置 proj4 完成投影转换。
 */
export declare class CRS {
   ...
   * 内置 EPSG 坐标系实例。
   */
  static readonly EPSG4326: CRS;
  static readonly EPSG4490: CRS;
  static readonly EPSG4214: CRS;
  static readonly EPSG4610: CRS;
  static readonly EPSG3857: CRS;
  /**
   * 内置坐标系（EPSG:4490 / EPSG:4214 / EPSG:4610）的 WKT 定义。
   */
  static readonly defaultWKTs: Record<string, string>;
  /**
   * 按 EPSG 码查找已注册的坐标系，未注册时返回 null。
   */
  static get(epsgCode: string): CRS | null;
  /**
   * 注册一个坐标系（以 EPSG 码去冒号并大写为键）。
   */
  static set(crs: CRS): void;
  /**
   * @param epsgCode - EPSG 码，如 'EPSG:4490'（CGCS2000）、'EPSG:4214'（北京54）、'EPSG:4610'（西安80）
   * @param wktOrExtent - 传入数组时作为投影范围 extent；传入字符串时作为 WKT 定义（缺省使用内置 defaultWKTs）
   * @param extent - 投影范围 [xmin, ymin, xmax, ymax]，优先级高于第二参数
   */
  constructor(
    epsgCode: string,
    wktOrExtent?: string | [number, number, number, number],
  ...
  readonly lngLatExtent: [number, number, number, number];
  renderWorldCopies: boolean;
  getExtent(): [number, number, number, number];
  getLngLatCenter(): [number, number];
   ...
   * 投影坐标 → WGS84 经纬度。
   */
  toWGS84(point: [number, number]): [number, number];
  /**
   * WGS84 经纬度 → 投影坐标。
   */
  fromWGS84(point: [number, number]): [number, number];
  /**
   * 经纬度 → 归一化投影坐标（x、y 为 0~1，z 为 0）。
   */
  fromLngLat(lnglat: LngLatLike): {
    x: number;
    y: number;
    z: number;
  };
...
export type MapOptions = {
  ...
  /**
   * 超图增强：地图坐标系（CRS）。
   * 可传内置 EPSG 码字符串（'EPSG:4326'、'EPSG:4490'、'EPSG:4214'、'EPSG:4610'、'EPSG:3857'），
   * 或 CRS 实例（可用 `new maplibregl.CRS(code, extent, wkt)` 注册自定义坐标系）。
   * 缺省为 CRS.EPSG3857。
   */
  crs?: string | CRS;
};
...
declare class Map$1 extends Camera {
   ...
   */
  /**
   * 超图增强：地图坐标系实例（由 MapOptions.crs 转换而来）。
   */
  readonly crs: CRS;
  /**
   * 超图增强：将 crs 参数（字符串或 CRS 实例）转换为 CRS 实例。
   * 字符串必须为已注册的 EPSG 码（内置或 CRS.set 注册），否则抛出异常。
   */
  toCRS(crs: string | CRS): CRS;
  /**
   * 超图增强：获取当前地图的坐标系（CRS）。
   */
  getCRS(): CRS;
  ...
  _createDelegatedListener(
    type: keyof MapEventType | string,
    layerId: string,
    listener: Listener,
  ): {
    layer: string;
...
export type GeoJSONSourceOptions = GeoJSONSourceSpecification & {
  ...
  /**
   * 超图增强：自定义投影脚本 URL。脚本被加载到 worker 中，
   * 需定义全局 customConvertPoint 对象（提供 projectXY(x, y, zoom) 与 toLngLat(x, y, zoom) 方法）进行坐标投影转换。
   */
  customprj?: string;
};
export type GeoJSONSourceIntenalOptions = {
  ...
  generateId?: boolean;
  /**
   * 超图增强：自定义投影脚本 URL（同 GeoJSONSourceOptions.customprj）。
   */
  customprj?: string;
};
/**
...
export declare class GeoJSONSource extends Evented implements Source {
  ...
  /**
   * 超图增强：设置 GeoJSON 源的自定义投影脚本（customprj）。
   * 脚本将被加载到 worker 中用于坐标投影转换（需定义全局 customConvertPoint 对象）。
   * @param customprj - 自定义投影脚本 URL
   */
  setCustomprj(customprj: string): void;
  /**
...
/**
 * 超图增强：内嵌的 proj4 投影转换库（maplibre-gl-enhance.js 内置完整 proj4 + mgrs）。
 */
export declare function proj4(
  fromProjection: string,
  toProjection: string,
  coordinates: [number, number],
): [number, number];
export declare function proj4(fromProjection: string, coordinates: [number, number]): [number, number];
export declare function proj4(coordinates: [number, number]): [number, number];
export declare namespace proj4 {
  /**
   * 定义或获取投影。
   */
  function defs(name: string, wkt?: string): void | unknown;
  /**
   * 将坐标数组转换为 {x, y} 点对象。
   */
  function toPoint(coordinates: number[]): {
    x: number;
    y: number;
  };
  /**
   * 经纬度（WGS84）→ MGRS 格网字符串。
   */
  function toMGRS(
    point:
      | [number, number]
  ...
  /**
   * MGRS 格网字符串 → WGS84 经纬度。
   */
  function fromMGRS(mgrs: string): [number, number];
  /**
   * 投影对象（含 project/inverse 方法）。
   */
  ...
  const Proj: {
    projections: Map<string, ProjProjection>;
  };
  const WGS84: string;
  const version: string;
  const transform: (
    source: string | ProjProjection,
    dest: string | ProjProjection,
    point: [number, number],
  ) => [number, number];
  const mgrs: unknown;
}
...
declare global {
  /**
   * 全局 GeoJSON 命名空间（最小声明）
   *
   * 根 tsconfig 的 compilerOptions.types 数组未包含 @types/geojson，
   * 此处为 Map 组件（MapClusterLayer/MapArc 等）的泛型参数提供全局类型。
   */
  namespace GeoJSON {
...
// ===== npm 包名 `maplibre-gl` 模块兼容 =====
// 根 tsconfig.json 的 compilerOptions.paths 将 `maplibre-gl` 映射到本文件
// （Map 组件等 `import MapLibreGL, { type Xxx } from 'maplibre-gl'` 使用；
// 运行时经 vite alias -> packages/vmap/src/maplibre-gl-shim.ts 桥接 window.maplibregl）。
// 官方 d.ts 无 default export，自引用默认导出提供与 UMD 全局等价的默认导入。
import * as maplibregl from './maplibre-gl-enhance';
